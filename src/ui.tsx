...
const selection = (value: {model: string; effort: string} | null) => value ? `${value.model} · ${value.effort}` : '—';
...
export function ConfigWizard({models, initial, language = 'en', onSave, onCancel}: {models: Model[]; initial?: Config; language?: Language; onSave: (config: Config) => Promise<void>; onCancel?: () => void}) {
  ...
  const [draft, setDraft] = useState<Config>(initial ?? {version: 1, language, normal: {model: fallback.model, effort: fallback.defaultReasoningEffort}, economy: {model: fallback.model, effort: fallback.defaultReasoningEffort}, policy: 'balanced'});
  const [step, setStep] = useState(0);
  const [error, setError] = useState('');
  const [busy, setBusy] = useState(false);
  const lang = draft.language;
  useInput((_input, key) => {if (key.escape && !busy) {if (step > 0) setStep(step - 1); else onCancel?.();}});
  const stages = [tr(lang, 'Language', '语言'), tr(lang, 'Normal model', 'Normal 模型'), tr(lang, 'Normal reasoning', 'Normal 推理强度'), tr(lang, 'Economy model (choose your preferred lower-cost model)', 'Economy 模型（请自行选择更节省额度的模型）'), tr(lang, 'Economy reasoning', 'Economy 推理强度'), tr(lang, 'Policy', '策略')];
  const values = step === 0 ? [{label: 'English', value: 'en'}, {label: '简体中文', value: 'zh-CN'}] : step === 1 || step === 3 ? models.map(m => ({label: `${m.displayName} (${m.model})`, value: m.model})) : step === 5 ? ['quality', 'balanced', 'saver'].map(value => ({value, label: value})) : (models.find(m => m.model === (step === 2 ? draft.normal.model : draft.economy.model)) ?? fallback).supportedReasoningEfforts.map(e => ({label: e.reasoningEffort, value: e.reasoningEffort}));
  const current = [lang, draft.normal.model, draft.normal.effort, draft.economy.model, draft.economy.effort, draft.policy][step];
  async function choose(value: string) {
    const next = structuredClone(draft);
    if (step === 0) next.language = value as Language;
    if (step === 1 || step === 3) {const model = models.find(m => m.model === value)!; next[step === 1 ? 'normal' : 'economy'] = {model: model.model, effort: model.defaultReasoningEffort};}
    if (step === 2 || step === 4) next[step === 2 ? 'normal' : 'economy'].effort = value;
...
export function TaskRow({task, next, selected, lang}: {task: Task; next: {model: string; effort: string} | null; selected: boolean; lang: Language}) {
  return <Box flexDirection="column"><Text color={selected ? 'cyan' : undefined} wrap="truncate">{selected ? '›' : ' '} {safeText(task.name)}  {task.priority.toUpperCase()} · {task.mode} · {task.status} · {task.id.slice(0, 8)}</Text><Text wrap="truncate">  {tr(lang, 'Current', '当前')}: {safeText(selection(task.current))} ({task.currentMode ?? 'unknown'})</Text><Text wrap="truncate">  {tr(lang, 'Next turn', '下一轮')}: {task.mode === 'off' ? tr(lang, 'Native client controls; history may be stale', '原生客户端决定；历史配置可能已过期') : safeText(selection(next))}</Text></Box>;
}
...
export function App({governor, initialTask}: {governor: Governor; initialTask?: string}) {
  ...
  if (screen === 'new') return <Box flexDirection="column" padding={1}><TextEntry title={tr(lang, 'New task name (Normal priority; current directory)', '新任务名称（Normal 优先级，使用当前目录）')} onCancel={() => setScreen('list')} onSubmit={name => {if (busy) return; void action(async () => {const created = await governor.createTask(name); setIndex(governor.tasks.findIndex(t => t.id === created.id)); setScreen('detail');});}} /><Text color="red">{safeText(notice)}</Text>{busy && <Text>{tr(lang, 'Creating…', '创建中…')}</Text>}</Box>;
  ...
  if (screen === 'priority' && task) return <Choices title={tr(lang, 'Priority', '优先级')} values={['high', 'normal', 'low'].map(value => ({value, label: value}))} onSelect={value => {void action(async () => {await governor.setPriority(task.id, value as Task['priority']); setScreen('list');});}} />;
  if (screen === 'mode' && task) return <Box flexDirection="column"><Choices title={tr(lang, 'Mode (future turns)', '模式（后续轮次生效）')} values={[{value: 'auto', label: tr(lang, 'Auto — Governor selects', 'Auto 自动选模')}, {value: 'manual', label: tr(lang, 'Manual — explicit model + effort', 'Manual 手动固定模型和思考强度')}, {value: 'off', label: tr(lang, 'Off — native client controls', 'Off 原生客户端控制')}, {value: 'keep', label: tr(lang, 'Keep — legacy Normal configuration', 'Keep 兼容模式：使用 Normal 配置')}]} onSelect={value => {if (busy) return; if (value === 'manual') {setScreen('manual-model'); return;} void action(async () => {await governor.setMode(task.id, value as Task['mode']); setScreen('list');});}} /><Text color="red">{safeText(notice)}</Text></Box>;
  ...
  const stageLabels = [tr(lang, 'NORMAL', 'NORMAL'), tr(lang, 'REASONING REDUCED', '推理强度降低'), tr(lang, 'ECONOMY', 'ECONOMY')];
  ...
  return <Box flexDirection="column" paddingX={1}>
    <Text bold color="cyan">CODEX RESOURCE GOVERNOR</Text><Text>{tr(lang, 'Policy', '策略')}: {governor.config.policy} {busy ? '…' : ''}</Text>
    ...
    <Text dimColor>{tr(lang, 'Enter Details/Prompt   P Priority   M Mode   O Open Codex', 'Enter 详情/输入   P 优先级   M 模式   O 打开 Codex')}</Text>
