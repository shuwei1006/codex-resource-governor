...
export const thresholds = {'quality-first': {reduce: 10, economy: 5}, balanced: {reduce: 20, economy: 10}, 'save-quota': {reduce: 40, economy: 20}} as const;
...
export type Decision = {stage: Stage; next: Selection | null; holds: Holds; remaining: number | null; reason: 'high' | 'keep' | 'quota' | 'held' | 'missing' | 'primary' | 'manual' | 'off'};
export function decide(task: Pick<Task, 'priority' | 'mode' | 'holds'> & Partial<Pick<Task, 'manualSelection'>>, config: Config, quota: Quota, models: Model[], now = Date.now() / 1000): Decision {
  ...
  for (const key of windowNames) {
    ...
    if (task.priority !== 'high' && task.mode === 'auto') {
      ...
      if (current.remaining < limits.economy && (config.policy !== 'quality-first' || task.priority === 'low')) stage = 2;
    }
  ...
  const reason = task.priority === 'high' ? 'high' : task.mode === 'keep' ? 'keep' : remaining === null ? 'missing' : stage > requested ? 'held' : stage ? 'quota' : 'primary';
  return {stage, holds, remaining, reason, next: stage === 2 ? {...config.economy} : stage === 1 ? lowerReasoning(config.primary, models) : {...config.primary}};
}
...
export function explain(task: Task, config: Config, decision: Decision, quota: Quota): string {
  ...
  const reasons = {
    ...
    high: zh ? '高任务优先级始终使用主力配置。' : 'High task priority always uses the Primary profile.',
    keep: zh ? 'Keep 模式始终使用主力配置。' : 'Keep mode always uses the Primary profile.',
    ...
    primary: zh ? '有效额度尚未触发降档阈值。' : 'The available quota does not cross a downgrade threshold.',
    quota: zh ? '最紧张的有效额度窗口触发了此策略。' : 'The tightest valid quota window triggers this policy.',
  };
  const rule = zh ? `${policyLabel(config.policy, config.language)}：低于 ${limits.reduce}% 时 中/低优先级任务降一级思考强度；低于 ${limits.economy}% 时${config.policy === 'quality-first' ? '低优先级任务' : '中/低优先级任务'}切节省配置。` : `${policyLabel(config.policy, config.language)}: below ${limits.reduce}%, reduce Medium/Low task reasoning effort by one supported level; below ${limits.economy}%, switch ${config.policy === 'quality-first' ? 'Low' : 'Medium/Low'} priority tasks to the Economy profile.`;
  const display = (s: Selection | null) => selectionLabel(s, config.language);
  return [windows || (zh ? '额度不可用。' : 'Quota unavailable.'), `${zh ? '额度策略' : 'Quota Policy'}: ${policyLabel(config.policy, config.language)}. ${zh ? '任务优先级' : 'Task Priority'}: ${priorityLabel(task.priority, config.language)}. ${zh ? '模式' : 'Mode'}: ${task.mode}.`, task.mode === 'auto' ? rule : '', `${zh ? '下一轮执行配置' : 'Next Model Profile'}: ${task.mode === 'off' ? (zh ? '原生控制' : 'Native control') : task.mode === 'manual' ? (zh ? '手动固定' : 'Manual selection') : profileLabel(decision.stage === 2 ? 'economy' : 'primary', config.language)}`, reasons[decision.reason], `${zh ? '最近提交' : 'Last submitted'} (${task.currentMode ?? 'unknown'}): ${display(task.current)}`, `${zh ? '下一轮' : 'Next turn'}: ${display(decision.next)}`, decision.stage === 1 && decision.next?.effort === config.primary.effort ? (zh ? '该模型无已知更低档位，保持当前思考强度。' : 'No known lower supported effort exists; reasoning stays unchanged.') : '', zh ? '阈值是本项目默认策略，并非 OpenAI 官方规则。' : 'Thresholds are project defaults, not official OpenAI rules.'].filter(Boolean).join('\n\n');
}
